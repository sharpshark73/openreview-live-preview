...
function collectBlockCodeRanges(value) {
  const ranges = [];
  let sourceOffset = 0;
  for (const token of marked.lexer(value)) {
    ...
    if (token.type === "code") {
      ranges.push([tokenStart, sourceOffset]);
    }
  }
  return ranges;
}
...
export function findSourceMarkdownMath(value = "") {
  const ignoredWarnings = [];
  const fenceRanges = collectFenceRanges(value, ignoredWarnings);
  const ignoredRanges = [
    ...fenceRanges,
    ...collectInlineCodeRanges(value, fenceRanges),
    ...collectBlockCodeRanges(value),
  ];
  const masked = Array.from(value);
  for (const [start, end] of ignoredRanges) {
    for (let offset = start; offset < end; offset += 1) {
      masked[offset] = POST_MARKDOWN_BLOCK_BOUNDARY;
    }
  }
  return findPostMarkdownMath(masked.join(""));
}

export function findMarkdownEmphasisEffects(value = "") {
  const effects = [];
  const visited = new Set();

  const visitTokens = (tokens, searchStart, searchEnd) => {
    let cursor = searchStart;
    for (const token of tokens ?? []) {
      if (!token || typeof token !== "object" || visited.has(token)) continue;
      visited.add(token);
      const raw = typeof token.raw === "string" ? token.raw : "";
      let start = raw ? value.indexOf(raw, cursor) : cursor;
      if (start < cursor || start >= searchEnd) start = cursor;
      const end = Math.min(searchEnd, start + raw.length);

      if (token.type === "em" || token.type === "strong") {
        const opening = raw.match(/^(\*{1,2}|_{1,2})/)?.[0] ?? "";
        const closing = raw.match(/(\*{1,2}|_{1,2})$/)?.[0] ?? "";
        if (opening && closing) {
          effects.push({
            type: token.type,
            start,
            ...
            marker: opening[0],
            markerOffsets: [
              ...Array.from({ length: opening.length }, (_, index) => start + index),
              ...Array.from(
                { length: closing.length },
                (_, index) => end - closing.length + index,
              ),
            ],
          });
        }
      ...
      if (Array.isArray(token.tokens)) {
        visitTokens(token.tokens, start, end);
      }
      if (Array.isArray(token.items)) {
        visitTokens(token.items, start, end);
      }
  ...
  let sourceOffset = 0;
  for (const token of tokens) {
    const tokenEnd = sourceOffset + (token.raw?.length ?? 0);
    if (Array.isArray(token.tokens)) {
      visitTokens(token.tokens, sourceOffset, tokenEnd);
    }
    if (Array.isArray(token.items)) {
      visitTokens(token.items, sourceOffset, tokenEnd);
    }
